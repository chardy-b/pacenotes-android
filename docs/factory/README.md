...
5. For each actual run only, create exactly one `docs/factory/runs/<WIL-ID>/<RUN-ID>.md` from [`RUN-LEDGER-TEMPLATE.md`](RUN-LEDGER-TEMPLATE.md), and create/update that ticket's `docs/factory/runs/<WIL-ID>/INDEX.md`; never create fake entries. `RUN-ID` is exactly `YYYYMMDDTHHMMSSZ-<short-head>-NN`, where `<short-head>` is the first seven lowercase hexadecimal characters of the exact 40-character head SHA and `NN` is a two-digit, zero-padded sequence starting at `01`.
